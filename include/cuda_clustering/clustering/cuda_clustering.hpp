...
class CudaClustering : public IClustering
{
  private:
    ...
    unsigned int *indexNear = NULL;
    unsigned int *indexFar  = NULL;

    float *nearInput = NULL;
    float *farInput  = NULL;
    
    unsigned int nearSize = 0;
    unsigned int farSize  = 0;


    
    //extractClusterParam_t ecp;
    
    extractClusterParam_t ecp_near;
    extractClusterParam_t ecp_far;

    cudaStream_t stream = NULL;

    void reallocateMemory(unsigned int sizeEC);

    void clustersToMarkers(
    float* cloud_in,
    unsigned int nCount,
    float* outputEC,
    unsigned int* index,
