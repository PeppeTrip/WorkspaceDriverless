...
ControllerNode::ControllerNode() : Node("clustering_node")
{
    ...
    cones_array_pub = this->create_publisher<visualization_msgs::msg::MarkerArray>("/perception/newclusters", 10);
    ...
    // Near cones
    cones_near->header.frame_id = frame_id;
    cones_near->ns = "cones_near";
    cones_near->id = 0;
    cones_near->type = visualization_msgs::msg::Marker::SPHERE_LIST;
    cones_near->action = visualization_msgs::msg::Marker::ADD;
    cones_near->scale.x = 0.2;
    cones_near->scale.y = 0.2;
    cones_near->scale.z = 0.2;
    cones_near->color.r = 0.0;
    cones_near->color.g = 1.0;
    cones_near->color.b = 0.0;
    cones_near->color.a = 1.0;

    // Far cones
    cones_far->header.frame_id = frame_id;
    cones_far->ns = "cones_far";
    cones_far->id = 0;
    cones_far->type = visualization_msgs::msg::Marker::SPHERE_LIST;
    cones_far->action = visualization_msgs::msg::Marker::ADD;
    cones_far->scale.x = 0.2;
    cones_far->scale.y = 0.2;
    cones_far->scale.z = 0.2;
    cones_far->color.r = 1.0;
    cones_far->color.g = 0.5;
    cones_far->color.b = 0.0;
    cones_far->color.a = 1.0;
...
void ControllerNode::loadParameters()
{
    ...
    // Range SPLIT
    declare_parameter("range_split_m", 4.0);
    get_parameter("range_split_m", range_split_m);
...
void ControllerNode::publishPc(float *points, unsigned int size,
                               rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pub)
{
    ...
    for (unsigned int i = 0; i < size; ++i) {
        pcl_cloud.points[i].x = points[4*i + 0];
        pcl_cloud.points[i].y = points[4*i + 1];
        pcl_cloud.points[i].z = points[4*i + 2];
    }
...
void ControllerNode::scanCallback(sensor_msgs::msg::PointCloud2::SharedPtr sub_cloud)
{
    ...
    //cones->points = {};
    cones_near->points.clear();
    cones_far->points.clear();
    unsigned int size = 0;
    float* tmp = NULL;
    ...
    // Original clustering call
    //this->clustering->extractClusters(inputData, inputSize, partialOutput, cones_near);

    this->clustering->extractClustersNearFar(
    inputData,
    inputSize,
    partialOutput,
    cones_near,
    cones_far,
    static_cast<float>(range_split_m)
    );
    ...
    // Publish cones original
    //cones->header.stamp = this->now();
    //if(cones->points.size() != 0)
    //    cones_array_pub->publish(*cones);

    cones_near->header.stamp = this->now();
    cones_far->header.stamp  = this->now();
    
    visualization_msgs::msg::MarkerArray arr;
    
    // Clear old markers (prevents RViz ghosts when count decreases)
    visualization_msgs::msg::Marker del;
    del.header = cones_near->header;
    del.action = visualization_msgs::msg::Marker::DELETEALL;
    arr.markers.push_back(del);
    
    // Publish both in ONE message
    arr.markers.push_back(*cones_near);
    arr.markers.push_back(*cones_far);
    
    cones_array_pub->publish(arr);
